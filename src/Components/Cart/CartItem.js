...
const CartItem = ({ item }) => {
  const [handelDeleteCart, show, handleClose, handleShow, handelDeleteItem] =
    DeleteCartHook(item);

  const [itemCount, onChangeCount, handeleUpdateCart] = UpdateCartHook(item);
          ...
          <Col sm="12" className="mt-1 d-flex">
            {item.color === "" ? null : (
              <div
                ...
                style={{ backgroundColor: `${item.colorD}` }}
              ></div>
            )}
          </Col>
        </Row>
